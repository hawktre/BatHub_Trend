...
for (i in 1:length(occ_data)) {
  ...
  path_name <- paste0("DataProcessed/results/stan/full/fits/",names(occ_data)[i], "_stan.rds")
  
  saveRDS(occ_stan, here(path_name))
}
# Write out needed files --------------------------------------------------

saveRDS(xmat_all, here("DataProcessed/results/stan/full/xmat_all.rds"))
saveRDS(xmat_cliff, here("DataProcessed/results/stan/full/xmat_cliff.rds"))
saveRDS(occ_data, here("DataProcessed/results/stan/full/occ_data.rds"))
st_write(nw_grid_all, here("DataProcessed/occurrence/nw_grid_all.shp"), append = F)
