## ARCHIVED DRAFT — reads from all_detections_join.csv (no longer in pipeline)
## Superseded by Code/formatting/00b_DetectionDataFormat.R
##
## ---------------------------
##
...
deployment <- read.csv(here("DataProcessed/detections/all_detections_join.csv"))
...
## Fix Date Columns
deployment$DeploymentDate <- as_datetime(deployment$DeploymentDate, format = "%m/%d/%y %H:%M") %>% as_date()
deployment$RecoveryDate <- as_datetime(deployment$RecoveryDate, format = "%m/%d/%y %H:%M") %>% as_date()
...
daymet_batch <- acoustics_wide %>% select(LocationName, Latitude, Longitude) %>% 
...
test2 <- daymet_all %>% filter(site %in% test$replicate)
...
acoustics_wide <- acoustics_wide %>% 
  left_join(daymet_clean, by = c("LocationName" = "site", "Night" = "date"))
