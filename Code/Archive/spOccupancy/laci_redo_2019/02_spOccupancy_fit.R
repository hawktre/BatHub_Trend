...
# Read in the data --------------------------------------------------------
bat.dat <- readRDS(here("DataProcessed/results/spOccupancy/laci_redo_2019/bat_dat.rds"))
...
for(s in 1:length(dimnames(bat.dat$y)[[1]])){
  ...
  ## Occurrence Formula
  occ.formula <- ~ scale(year) + precip + elev + forest
  ...
  saveRDS(fit, here(paste0("DataProcessed/results/spOccupancy/laci_redo_2019/fits/", filename)))
}
