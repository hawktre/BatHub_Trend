...
## spOccupancy
spOcc_summ <- readRDS(here("DataProcessed.nosync/spOccupancy/covars_summary.rds"))
...
#Alphas
alpha_summ <- alpha_summ %>% mutate(analysis = "OR|WA|ID", model = "Stan") %>% select(-q50)
alpha_summ_sens <- alpha_summ_sens %>% mutate(analysis = "OR|WA Only", model = "Stan") %>% select(-q50)
alpha_spocc <- spOcc_summ %>% 
  filter(type == "Occurrence") %>% 
  rename("alpha" = covariate) %>% 
  mutate(analysis = "OR|WA|ID",
         model = "spOccupancy",
         alpha = case_when(alpha == "Cliff/Canyon Cover" ~ "cliff_cover",
                           alpha == "Year" ~ "year",
                           alpha == "Forest Cover" ~ "forest_cover",
                           alpha == "Precipitation" ~ "precip",
                           alpha == "Elevation" ~ "elevation",
                           T ~ alpha)) %>% 
  select(spp, alpha, mean, q2.5, q25, q75, q97.5, analysis, model)

alpha_full <- bind_rows(alpha_summ, alpha_summ_sens, alpha_spocc) 


  
alpha_full %>% 
  filter(!(alpha %in% c("int01", "int02", "alpha_auto", "(Intercept)")),
         !(spp %in% exclude),
         !model == "spOccupancy") %>% 
  mutate(spp = toupper(spp),
         alpha = factor(alpha, levels = c("forest_cover", 
                                          "precip",
                                          "elevation",
                                          "cliff_cover",
                                          "year"),
  ...
  mutate(spp = toupper(spp),
         alpha = factor(alpha, levels = c("forest_cover", 
                        ...
                        labels = c("Forest Cover",
                                    ...
                                    "Cliff Cover",
                                    "Year"))) %>%
ggplot(aes(x = spp, y = mean, group = model))+
  geom_errorbar(aes(ymin = q2.5, ymax = q97.5), width = 0, size = 0.75,
                position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = q25, ymax = q75, color = model), width = 0,
                size = 1.5, position = position_dodge(width = 0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
  theme_bw() +
  facet_grid(alpha ~ ., scales = 'free') +
...
#Betas
beta_summ <- beta_summ %>% mutate(analysis = "OR|WA|ID", model = "Stan")
beta_summ_sens <- beta_summ_sens %>% mutate(analysis = "OR|WA Only", model = "Stan")

beta_spocc <- spOcc_summ %>% 
  filter(type == "Detection") %>% 
  rename("betas" = covariate) %>% 
  mutate(analysis = "OR|WA|ID",
         model = "spOccupancy",
         betas = case_when(betas == "dayl" ~ "daylight",
                           betas == "(Intercept)" ~ "Intercept",
                           T ~ betas)) %>% 
  select(spp, betas, mean, q2.5, q25, q75, q97.5, analysis, model)

beta_full <- bind_rows(beta_summ, beta_summ_sens, beta_spocc) 
...
psi_spocc <- readRDS(here("DataProcessed.nosync/spOccupancy/psi_hat.rds")) 

psi_spocc <- psi_spocc %>% select(year, mean, everything()) %>% mutate(analysis = "OR|WA|ID",
                                                                       model = "spOccupancy",
                                                                       year = as.character(year))

all_psi_stan <- bind_rows(psi_full, psi_sens) %>% mutate(model = "Stan")

names(psi_spocc) <- names(all_psi_stan)

all_psi <- bind_rows(psi_spocc, all_psi_stan)
...
all_psi %>% 
  filter(!(spp %in% exclude),
         analysis == "OR|WA|ID") %>% 
  mutate(spp = toupper(spp)) %>% 
ggplot(aes(x = years, y = mean_psi, group = model))+
  geom_errorbar(aes(ymin = q2.5, ymax = q97.5), width = 0, size = 0.75,
                position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = q25, ymax = q75, color = model), width = 0,
                size = 1.5, position = position_dodge(width = 0.5)) +
